...
const MENU_DATA={
  ...
  life:[
    {title:'포인트',items:[
      ...
      {id:'l2',label:'올원룰렛',icon:'disc-3',shape:'circle',bg:'var(--obj-green)'},
...
function AllMenuScreen({onBack,big}){
  ...
  return (
    <div className={big?'allone-bigtext':undefined} style={{flex:1,minHeight:0,display:'flex',flexDirection:'column',background:'var(--surface-app)'}}>
      ...
      <div style={{flex:1,minHeight:0,overflowY:'auto',padding:'20px var(--gutter) 28px',display:'flex',flexDirection:'column',gap:'var(--card-gap)'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:10,paddingTop:20}}>
          ...
          <span style={{fontSize:'var(--fs-micro)',color:'var(--text-subtle)'}}>올원뱅크 v6.2.1 · 준법감시인 심의필 2026-0000</span>
        </div>
      </div>
