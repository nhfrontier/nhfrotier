...
const SHORTCUTS=()=>[
  ...
  {id:'룰렛',label:'올원룰렛',shape:'circle',bg:'var(--obj-green)',icon:G('disc-3')},
...
function HomeScreen({onOpenMenu,big,setBig}){
  ...
  return (
    <div className={big?'allone-bigtext':undefined} style={{flex:1,minHeight:0,display:'flex',flexDirection:'column'}}>
      <header style={{display:'flex',alignItems:'center',gap:8,padding:'4px 12px 8px 20px',flex:'none',background:'var(--surface-app)'}}>
        <span style={{flex:1,fontSize:'var(--fs-title-3)',fontWeight:800,letterSpacing:'var(--ls-title)',color:'var(--text-strong)'}}>홍길동님</span>
      ...
      <div style={{flex:1,minHeight:0,overflowY:'auto',padding:'0 var(--gutter) 20px',display:'flex',flexDirection:'column',gap:'var(--card-gap)'}}>
        ...
        <div style={{display:'flex',alignItems:'baseline',gap:10,paddingTop:12}}>
          <span style={{fontSize:'var(--fs-title-3)',fontWeight:800,color:'var(--text-strong)',letterSpacing:'var(--ls-title)'}}>올원뱅크</span>
        ...
        <Card>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <span style={{width:44,height:44,borderRadius:'var(--r-pill)',background:'var(--teal-500)',color:'#fff',
              display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,fontWeight:800,letterSpacing:'-.03em'}}>올원</span>
            <span style={{flex:1,minWidth:0}}>
              <span style={{display:'block',fontSize:'var(--fs-body-lg)',fontWeight:700,color:'var(--text-strong)'}}>올원 주거래우대통장</span>
              <span style={{display:'block',fontSize:'var(--fs-body-sm)',color:'var(--text-muted)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>올원뱅크 302-2212-1373-21 <u style={{cursor:'pointer'}} onClick={()=>setToast('계좌번호를 복사했어요')}>복사</u></span>
            </span>
        ...
        <Card>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            <span style={{flex:1,fontSize:'var(--fs-body-lg)',fontWeight:700,color:'var(--text-strong)'}}>올원 광운대 동문회</span>
