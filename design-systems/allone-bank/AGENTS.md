# AGENTS.md — 올원뱅크 (AllOne Bank) Design System

**Read this file first. It is the entry point.** You are designing for 올원뱅크, a Korean mobile banking
...
- [ ] The layout still works with `.allone-bigtext` on the root (큰글 mode)
...
- **No logo.** The brand renders as the word 올원 set in type. Do not draw a mark.
