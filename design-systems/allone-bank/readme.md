# 올원뱅크 (AllOne Bank) Design System
...
The only source material supplied was **ten Android screenshots of the 올원뱅크 app**
(`uploads/KakaoTalk_20260830_131140555*.jpg`). No codebase, Figma file, brand guide, logo, or font
binary was provided.

Reproducing a brand's logo, mascot, wordmark or brand colours from screenshots is not
...
- **Substitutes its own visual identity** — the teal/amber palette, the type ramp, the shape and
  motion language are invented placeholders. None of it is the real 올원뱅크 CI, and none of it is
  traced from the captured screens.
- **Ships no logo.** No mark was supplied and none was drawn. The brand renders as the word 올원 set
  in type wherever a mark would go (see `guidelines/brand-wordmark.html`).

Everything visual here is a **prototyping stand-in**. Swap it for the licensed 올원뱅크 / NH assets
before any of this leaves the prototype.
...
### Colour
Brand **올원 Teal** (`--teal-500 #0B8478`) is the single action colour — primary buttons, filled toggles,
...
Amounts use tabular figures always (`.allone-num`), with the unit suffix (원 / P / %) dropped to 72% size and
one weight step so the figure stays the focal point.

**큰글 mode** is a first-class requirement, not an afterthought: `.allone-bigtext` rescales the whole ramp
...
human or an agent reads says 올원 / AllOne.
