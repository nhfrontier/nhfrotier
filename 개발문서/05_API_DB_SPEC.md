...
**(2026-09-08) `aiRewrite` 뒤에도 식별자는 이어진다.** 교체 조각의 첫 요소가 원래 `data-nh-id`를 잇고, 조각 안의 자손에는 baking 시점에 새 식별자를 붙인다(걷는 순서가 고정이라 읽을 때마다 같은 값이다). 그리고 편집·의견 앵커의 **대상 존재 검증은 `screen_elements`가 아니라 baking된 문서**를 기준으로 한다 — 그 표는 생성 시점의 지문이라 교체 조각의 자손은 없고 교체·되돌리기로 사라진 요소는 남아 있어, 표를 믿으면 "201로 받아 놓고 반영은 안 되는" 편집이 생긴다. 운영 백엔드 `ScreenBaker.hasElement()`.
