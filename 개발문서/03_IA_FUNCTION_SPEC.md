...
→ **제품 설명 히어로 + 나의 작업, 검토 대기, 최근 Version, 최근 활동** (2026-09-08 변경)

- **최근 프로젝트 제거**: 바로 옆 S02 Project List가 같은 목록을 전부 보여주고, 카드가 프로젝트별로 갈라지지도 않아 중복이었다.
- **히어로 추가**: 첫 화면에 제품이 무엇을 해주는 도구인지 설명할 지점이 없었다. 인사말("안녕하세요, ○○○님")을 히어로가 대체한다. 3단계(기획안 → 화면 초안 / 의견 → AI 정리 / 결정 → 새 Version)를 순차 강조해 제품의 흐름 자체를 보여준다.
- 화면상 제품명을 `NH뚝딱협업스튜디오` → **`NH 위드캔버스`** 로 통일했다. 문서 표기는 아직 옛 이름이 섞여 있다.
