...
- **함께 쓰는 것**: `@vue/test-utils`(컴포넌트 마운트) · `happy-dom`(DOM 환경). ~~순수 함수만 덮으므로 DOM 이 필요 없다~~ → **2026-09-08 추가.** 순수 함수 테스트만으로는 화면에서 나는 문제를 못 잡는다는 것이 실제로 드러났다 — `evidence_source` 를 코드값 그대로 노출하던 버그는 브라우저에서만 잡혔다.
- **왜 `jsdom` 이 아닌지**: `happy-dom` 이 가볍고 우리가 쓰는 범위(마운트·이벤트·속성 읽기)를 다 덮는다. 무거운 쪽을 고를 이유가 없다.
...
- **컴포넌트에서 지키는 것**: `sandbox` 가 `allow-scripts` 하나뿐인지(편집 모드에서도), 두 검토의 결정 축이 다른지(`준법 검토 요청`은 FR-14 고유), 근거 출처가 코드값이 아니라 사람이 읽는 말인지, 답글이 뿌리 아래로 접히고 핀이 뿌리에만 붙는지, 앵커를 잃은 의견이 숨겨지지 않는지, 스타일 값 가드가 반영 버튼을 막는지.
- **검증 방법**: 통과만으로는 부족해서 **변이 테스트로 확인했다.**
  - 순수 함수: `event.source` 대조 제거 · CSP `img-src` 개방 · 헤더 가드 제거 → 각각 1건씩 실패
  - 컴포넌트: `sandbox` 에 `allow-same-origin` 추가 → **3건 동시 실패** / UX 검토에 준법 축 추가 → 1건 / 답글을 뿌리와 나란히 세움 → 2건
- **뷰 통합에서 지키는 것**: 백엔드 응답을 **실제 모양 그대로** 두고 화면이 그것을 읽는지 본다. `GET /projects/{id}` 가 `{project, myRole, members}` 인 것, `/members` 가 배열인 것, `screens.status` 가 `PLANNED` 인 것. 어떤 요청을 어떤 본문으로 보내는지도 확인한다(검토 실행이 Version 경로인지, 의견에 `nhId`·`parentId` 를 어떻게 싣는지).
- **테스트가 실제로 찾아낸 버그**: 편집 모드를 껐다 켜면 선택이 남은 채 `ElementPanel` 이 다시 마운트되는데, `watch` 에 `immediate` 가 없어 입력칸이 빈 채로 떴다.
- **과거 버그 재현으로 검증**: 실제로 냈던 버그 셋(`GET /projects/{id}` 를 `Project` 로 가정 · 상태값을 `PENDING` 으로 가정 · `/members` 를 따로 호출)을 코드에 다시 넣어 각각 1~2건이 실패하는 것을 확인했다. 이 테스트들이 **그때 있었다면 막았을** 버그다.
- **왜 Playwright 를 아직 넣지 않았는지**: 지금까지 낸 버그는 전부 **응답 형태를 잘못 가정한 것**이었고, 그것은 뷰를 마운트하고 fetch 를 가짜로 두면 의존성 추가 없이 잡힌다. 브라우저 바이너리를 받고 스택을 띄워야 하는 비용은 라우팅·여러 화면을 오가는 흐름에서 실제 문제가 났을 때 치른다.
- **아직 없는 것**: 여러 화면을 오가는 E2E. 화면 사이 이동과 새로고침 동작은 여전히 **브라우저 수동 확인**에 의존한다.
