...
- **해결**: 로그의 `조치:` 줄을 따른다. 호스트에서 `chown -R <uid>:<gid> <경로>` 하거나,
  소유권을 바꿀 수 없는 NFS 라면 `docker-compose.yml` 의 `user:` 로 실행 uid 를 맞춘다.
- **userns-remap / rootless Docker 를 쓴다면**: 컨테이너 안 uid 10001 이 호스트에서는 다른 값(예: 175537)이다.
  `chown` 은 **호스트에서 실행하는 명령**이므로 컨테이너 uid 를 그대로 쓰면 틀린다.
  로그가 `(uid 10001, 호스트 uid 175537)` 처럼 둘을 함께 찍으므로 **호스트 uid 쪽**을 쓴다.
  매핑을 해석하지 못하면 chown 명령 대신 "`/proc/self/uid_map` 을 보고 번역하라"고 안내한다 —
  틀린 명령을 주지 않기 위한 것이며, 이때는 관리자가 직접 번역해야 한다.
